"""Set Version 3 Division (:file:`set_version_3div.py`)
...
def build_version_tree_3div(
    ...
    for vers in formatted_all_versions:
        ...
        assert len(vers_split) == 3, f"| Expect at 3 versions split parts, got {vers_split}"
        ...
        unsorted_version_tree[vers_split[0]][vers_split[1]].append(vers_split[2])
    ...
    for major in sorted(unsorted_version_tree.keys(), key=basic_sort_lambda):
        ...
        for minor in sorted(unsorted_version_tree[major].keys(), key=basic_sort_lambda):
            sorted_version_tree[major][minor] = sorted(
                unsorted_version_tree[major][minor],
                key=basic_sort_lambda,
            )

    return sorted_version_tree


def fine_sorted_tree_3div_to_list(version_tree: dict[str, dict[str, list[str]]]):
    ...
    for major in version_tree:
        for minor in version_tree[major]:
            for patch in version_tree[major][minor]:
                fine_sorted_list.append(f"{major}.{minor}.{patch}")
...
FINE_SORT_VERSION_TREE = build_version_tree_3div(FORMATTED_ALL_VERSIONS)
"""The version tree with fine sorting"""

FINE_SORTED_LIST = fine_sorted_tree_3div_to_list(FINE_SORT_VERSION_TREE)
...
def bump_version_3div(
    ...
    and it should be looked like ['0', '3', '0'].
    ...
    Returns:
        tuple[str, str, str]: The bumped version number
    """
    assert (
        len(version_split) == 3
    ), f"| The version number should be split by dot and have only 3 parts: {version_split}."

    if bump_type == "patch":
        version_new_split = version_split[:2] + [str(int(version_split[2]) + 1)]

    elif bump_type == "minor":
        version_new_split = version_split[:1] + [str(int(version_split[1]) + 1)] + ["0"]

    elif bump_type == "major":

        version_new_split = [str(int(version_split[0]) + 1)] + ["0", "0"]

    else:
        raise ValueError(
            "| The bump type should be one of the following: "
            + f"major, minor, patch, or skip, but got: '{bump_type}'."
        )

    assert (
        len(version_new_split) == 3
    ), f"| The bumped version number should have 3 parts: {version_new_split}."
    return (version_new_split[0], version_new_split[1], version_new_split[2])


class SetVersionArgs(argparse.Namespace):
    """The arguments for :file:`set_version.py`"""

    release: Literal["stable", "check"]
    """Choose release type: 
    ...
    bump: Literal["major", "minor", "patch", "skip"]
...
if __name__ == "__main__":
    ...
    arguments: SetVersionArgs = parser.parse_args()  # type: ignore
    ...
    if arguments.release == "check":
        ...
        if len(VERSION_SPLIT) == 3:
            ...
            raise ValueError(f"| The version number should have 3 parts, but got: {VERSION_SPLIT}.")
    ...
    VERSION_SPLIT = bump_version_3div(VERSION_SPLIT, arguments.bump)
